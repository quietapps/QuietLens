...
struct OnboardingView: View {
    ...
    var body: some View {
        ZStack {
            ...
            VStack(spacing: FL.S.s5) {
                ...
                Text("Welcome to Quiet Lens").font(FL.T.display())
                Text("Dim everything except the window you're working in. Quiet Lens needs Accessibility access to detect which window is focused.")
                    .font(FL.T.bodyR())
                ...
                if !trusted {
                    GlassPanel {
                        VStack(alignment: .leading, spacing: 10) {
                            ...
                            stepRow(2, "Find Quiet Lens and click the − button to remove it")
                            stepRow(3, "Click + and add Quiet Lens.app from /Applications")
                            stepRow(4, "Toggle Quiet Lens ON in the list")
                        }
                        .padding(FL.S.s4)
                    }
                    .frame(maxWidth: 520)
    ...
    private func recheck() {
        ...
        if t {
            ...
            checkFailedMessage = "macOS still reports access not granted. Remove Quiet Lens from the Accessibility list and add it again, then click below."
        }
    }
